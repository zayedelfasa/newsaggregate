...
<div class="space-y-5 px-4 py-4">
	...
	{#if activeMovies || sectionMovies}
		...
		<section><div class="mb-2 flex items-center justify-between"><h2 class="text-xs font-bold uppercase tracking-wide text-gray-500 dark:text-neutral-400">{activeTitle || sectionTitle}</h2>{#if data.search}<span class="text-[10px] text-gray-400">Halaman {data.page}</span>{/if}</div>{#if movies.length}<div class="space-y-2">{#each movies as movie (movie.id)}<MovieListItem {movie} />{/each}</div>{:else}<p class="rounded-xl border border-gray-100 p-5 text-center text-sm text-gray-500 dark:border-neutral-800">Film tidak ditemukan.</p>{/if}{#if data.search && movies.length >= 20}<a href={`/hiburan?q=${encodeURIComponent(data.query)}&page=${data.page + 1}`} class="mt-3 block rounded-xl border border-gray-200 bg-white px-4 py-3 text-center text-xs font-bold text-red-500 dark:border-neutral-800 dark:bg-neutral-900">Muat halaman berikutnya →</a>{/if}</section>
	{:else}
		<div class="space-y-1.5">
